...
class Simulation
  ...
  def station
    loop do
      ...
      when 'report'
        show_trains_on_station(Station.find(command.shift))
      when 'all'
  ...
  def train_select(train_number)
    ...
    loop do
      ...
      when 'connect'
        case command.shift
        when 'cargo'
          train.connect_carriage(CargoCarriage.new(command.shift.to_i))
        when 'passenger'
          train.connect_carriage(PassengerCarriage.new(command.shift.to_i))
        end
      when 'disconnect'
        train.disconnect_carriage
      when 'fill'
        train.select_carriage(command.shift.to_i).fill(command.shift.to_i)
      when 'forward'
        train.forward
      when 'back'
        train.back
  ...
  def show_carriages(train)
    ...
    train.each_carriage do |carriage|
      puts carriage.number.to_s.ljust(8) +
           carriage.class.to_s[0..-9].ljust(11) +

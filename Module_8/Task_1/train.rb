...
class Train
  ...
  def each_carriage(&block)
    @rolling_stock.each { |carriage| block.call(carriage) }
  end
  ...
  def validate!
    if id.nil? || id !~ NUMBER_FORMAT
      raise StandardError.new('Invalid train number: XXX-XX/XXXXX')
    end
    raise StandardError.new('Invalid rate') if rate.nil? || rate <= 0
    raise StandardError.new('Invalid route') unless  route.is_a?(Route)
    true
  end
end
